...
function api(path,options){return fetch(path,options).then(async response=>{if(!response.ok)throw new Error(await response.text());return response.json()})}
...
async function loadUpdatedRouteIndex(){const response=await fetch('/api/catalog/route-index',{cache:'no-store'});if(response.status===404)return null;if(!response.ok)throw new Error(await response.text());const result=await response.json();if(!result?.bus?.kmb||!result?.bus?.ctb||!result?.gmb)throw new Error('本機更新路線索引格式不正確');localCatalog.routeIndex=result;return result}
...
async function requestRouteRefresh(slot,kind,refreshSharedStops=true,force=false){const widget=widgetDrafts[slot],route=kind==='bus'?widget.bus.route_id:widget.gmb.route_code,operator=kind==='bus'?widget.bus.operator:'',key=overrideKey(kind,operator,route),promiseKey=`refresh:${key}`;if(!route)return null;if(!force&&localCatalog.overrides.has(key)){applyRouteOverride(slot,localCatalog.overrides.get(key));return localCatalog.overrides.get(key)}if(localCatalog.overridePromises.has(promiseKey))return localCatalog.overridePromises.get(promiseKey);const promise=fetch('/api/catalog/route-refresh',{method:'POST',headers:{'Content-Type':'application/json',[csrfHeader]:csrfToken},body:JSON.stringify({kind,operator,route,refresh_routes:false,refresh_shared_stops:refreshSharedStops})}).then(async response=>{const body=await response.text();if(!response.ok)throw new Error(body);return JSON.parse(body)}).then(updated=>{localCatalog.overrides.set(key,updated);applyRouteOverride(slot,updated,true);return updated}).finally(()=>localCatalog.overridePromises.delete(promiseKey));localCatalog.overridePromises.set(promiseKey,promise);return promise}
async function loadRouteOverrideForSlot(slot){const widget=widgetDrafts[slot],kind=widget.type==='bus_eta'?'bus':widget.type==='gmb_eta'?'gmb':'',route=kind==='bus'?widget.bus.route_id:kind==='gmb'?widget.gmb.route_code:'',operator=kind==='bus'?widget.bus.operator:'';if(!kind||!route)return;const key=overrideKey(kind,operator,route);if(localCatalog.overrides.has(key)){applyRouteOverride(slot,localCatalog.overrides.get(key));return}if(localCatalog.overridePromises.has(key))return;const query=`kind=${kind}&route=${encodeURIComponent(route)}${kind==='bus'?`&operator=${encodeURIComponent(operator)}`:''}`;const promise=fetch(`/api/catalog/route-override?${query}`).then(async response=>{if(response.status===404)return null;if(!response.ok)throw new Error(await response.text());return response.json()}).then(async result=>{if(result){localCatalog.overrides.set(key,result);applyRouteOverride(slot,result);return result}if(localCatalog.routeIndex?.updated_at&&updatedRouteExists(kind,operator,route))return requestRouteRefresh(slot,kind,true);return null}).catch(error=>{widgetErrors[slot]=`未能準備最新站牌：${error.message}`;return null}).finally(()=>localCatalog.overridePromises.delete(key));localCatalog.overridePromises.set(key,promise);await promise;if(slot===expandedSlot)renderWidgetCards()}
